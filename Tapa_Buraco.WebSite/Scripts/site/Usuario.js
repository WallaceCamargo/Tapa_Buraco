...
function CarregarGrid() {
    $("#grid").LoadGrid({
        Parameters: {
            nome: $("#textBoxConsultarUsuario").val(),
        }
    });
}
...
function CriarGrid() {
    $("#grid").CreateGrid({
        options: {
            ...
            jqueryuiTheme: true,
            tableId: 'GridUsuarios',
            toolbar: {
                hoverAnimation: true, //Enable/disable small animation on mouse hover to a toolbar item.
                hoverAnimationDuration: 60, //Duration of the hover animation.
                hoverAnimationEasing: undefined, //Easing of the hover animation. Uses jQuery's default animation ('swing') if set to undefined.

                items: MostrarButtonAdicionarUsuario(),
            },
        },
        actions: {
            listAction: '/Usuario/GetAll',
        ...
        fields: {
            ...
            OpenDialogEdit: {
                ...
                display: function (item) {
                    //if ($("#hiddenEdit").val() == "EditarUsuario") {
                    if (true) {
...
function MostrarButtonAdicionarUsuario() {
    var array = [];
    //if ($("#hiddenAdd").val() == "AdicionarUsuario") {
    if (true) {
        var item = {
            ...
            position: 'left',
            click: function () {
                window.location = url_Base_Site + "/Usuario/Cadastro/0";
            }
        }
        array.push(item);
    } else {
...
function AbriEditar(item) {
    window.location = url_Base_Site + "/Usuario/Cadastro/" + item.record.ID;
}

function AbrirModalExcluir(item) {

    $.ajax({
        url: '/Usuario/_ModalExluir',
        success: function (data) {

            $('#dialog-ExcluirUsuario').html(data);
            ...
            if (item != null) {
                $("#divContentConfirmaExclusao").html("<p>Deseja excluir o usuario \"" + item.record.NOME + "?</p>");
            }
